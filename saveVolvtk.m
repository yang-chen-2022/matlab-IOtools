function saveVolvtk(VOL0,nfich,type,choix,spacing)
...
% 10/02/2016 Y.CHEN
%
% comment 13/02/2017 Yang Chen:
% the saveVolvtk is used for the application of Paraview software, which
% does not use the conventionale X Y (inverted), that is why we "permute"
% the 1st and the 2nd dimensions when saving VTK files.
% To summarize, what the user needs to be aware is that the binaries in VTK
% files are XY-inverted ordered comparing to the binaries inRAW files
%
...
%     [nx, ny, nz] = size(VOL0);
    ...
    % dataType
    VOL0 = permute(VOL0,[2 1 3]);%"permute" to adjust the fread with the conventional coordinates (x<=>column, y<=>row)
...
%     [nx, ny, nz] = size(VOL0{1});
    ...
    for ifield = 1:nfields
        ...
        fprintf(fid, 'LOOKUP_TABLE default\n');
        fwrite(fid,permute(VOL0{ifield},[2,1,3]),type,0,'b');%"permute" to adjust the fread with the conventional coordinates (x<=>column, y<=>row)
    end
